...
// `request/context` event), and returns a JSON payload. Per-file aggregation
// results are memoized in memory by file mtime/size AND persisted under the
// plugin's own `./.cache/` directory, so repeated panel refreshes are cheap
// and a dsh web restart doesn't force a full re-scan: the cache is re-loaded
// on boot, and every file is still stat'ed and compared by mtime/size before
// its cached result is trusted — a changed or appended session always
...
import { addBucket, aggregateEvents, emptyBucket, parseAggResult, serializeAggResult, shapePayload } from "./stats.js";
...
/** Lines that can carry usage, model attribution, or search-sidecar call records. */
const RELEVANT_LINE = /"(?:assistant\/message|request\/header|request\/context|web\/deepseek-search-llm-request)"/;
...
function mergeBuckets(byDay, byModel, searchByDay, result) {
  for (const [day, count] of result.searchByDay || []) searchByDay.set(day, (searchByDay.get(day) || 0) + count);
...
/**
 * Per-file aggregation cache, keyed by absolute path and invalidated by
 * mtime/size. Lives in memory for fast reuse and is ALSO persisted to disk
 * under the plugin's own root (`<plugin root>/.cache/aggregations.json`) so a
 * dsh web restart doesn't force a full re-scan: on boot the cache is
 * pre-populated from disk, then every file is still stat'ed and compared
 * before use, so a changed or newly-appended session always invalidates
 * exactly that file's entry.
 *
 * The cache deliberately lives INSIDE this package (resolved from
 * `import.meta.url`), never under `$DSH_HOME` or the session-log directory:
 * it avoids touching any harness data and means `dsh plugin remove` deletes
 * the whole plugin folder — cache included — for a clean uninstall. A
 * `dsh plugin update` replaces the folder too, so an upgrade simply drops the
 * cache (next open does one full re-scan, then it rebuilds).
 */
const fileCache = new Map();
let cacheLoaded = false;

/** Directory containing this module (lib/); the package root is one level up. */
const PLUGIN_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "..");

/** Cache file lives under the plugin's own `./.cache/`, never under $DSH_HOME. */
function cacheFilePath() {
  return join(PLUGIN_ROOT, ".cache", "aggregations.json");
}

/** Pre-populate `fileCache` from the persisted cache (best-effort, never throws). */
  ...
  try {
    ...
    if (data && data.files) {
      for (const [path, rec] of Object.entries(data.files)) {
        // Records from before the searchByDay counter existed are re-aggregated
        // so per-day search-call counts are not silently missing.
        if (typeof rec.mtimeMs === "number" && typeof rec.size === "number" && rec.result && Array.isArray(rec.result.searchByDay)) {
          fileCache.set(path, { mtimeMs: rec.mtimeMs, size: rec.size, result: parseAggResult(rec.result) });
        }
      }
    }
  } catch {
    // no/corrupt cache — start empty; computeStats repopulates
  }
}

/** Persist `fileCache` to disk atomically (best-effort, never throws). */
async function storeCache() {
  if (!cacheLoaded) return;
  try {
    const files = {};
    for (const [path, rec] of fileCache) {
      files[path] = { mtimeMs: rec.mtimeMs, size: rec.size, result: serializeAggResult(rec.result) };
    }
    const target = cacheFilePath();
    await mkdir(dirname(target), { recursive: true });
    const tmp = `${target}.${process.pid}.tmp`;
    await writeFile(tmp, JSON.stringify({ v: 1, files }));
    await rename(tmp, target);
  } catch {
    // persistence is best-effort; a failed write only costs a slower next boot
  }
}
...
/** Compute the global token-usage stats payload (per-file memoized + persisted). */
async function computeStats(ctx) {
  ...
  for (const file of files) {
    ...
    if (result !== null) mergeBuckets(byDay, byModel, searchByDay, result);
  }
